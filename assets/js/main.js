...
const DocsTOC = (() => {
  class TreeTOC {
      ...
      itemsArray.forEach((item, index) => {
        ...
        if (item.classList.contains('doc-toc__item--has-children')) {
          ...
          if (children.length > 0) {
            ...
            childrenContainer.className = 'doc-toc__children doc-toc__children--collapsed';
    ...
    syncTOCState(sourceItem) {
      ...
      const href = link.getAttribute('href');
      const isExpanded = sourceItem.classList.contains('doc-toc__item--expanded');

      // Find the matching item in other TOC instances
      document.querySelectorAll(`.doc-toc__link[href="${href}"]`).forEach(otherLink => {
        const otherItem = otherLink.closest('.doc-toc__item');
        if (otherItem && otherItem !== sourceItem) {
          const otherChildren = otherItem.querySelector('[data-toc-children]');
          const otherFolderIcon = otherItem.querySelector('[data-toc-folder] i');
    ...
    expandToSection(sectionId) {
      ...
      document.querySelectorAll(`.doc-toc__link[href="#${sectionId}"]`).forEach(link => {
        const item = link.closest('.doc-toc__item');
        if (!item) return;

        // Expand parent H2 if this is a child item
        const parentItem = item.parentElement?.closest('.doc-toc__item--has-children');
        if (parentItem && !parentItem.classList.contains('doc-toc__item--expanded')) {
    ...
    updateActiveState(activeId) {
      ...
      document.querySelectorAll(`.doc-toc__link[href="#${activeId}"]`).forEach(link => {
      ...
      window.addEventListener('scroll', () => {
        if (!ticking) {
          requestAnimationFrame(() => {
            const sections = document.querySelectorAll('.doc-article__content h1[id], .doc-article__content h2[id], .doc-article__content h3[id], .doc-article__content h4[id]');
            const header = document.querySelector('.header');
            const headerHeight = header ? header.offsetHeight : 80;
            ...
            if (currentSection && currentSection !== this.activeSection) {
              ...
              // this.expandToSection(currentSection); // User requested to remove auto-expand on scroll
    ...
    tables.forEach(table => {
      ...
      if (isMobile) {
        ...
        if (table.scrollWidth > wrapper.clientWidth) {
          ...
          indicator.innerHTML = '<i class="fas fa-arrows-left-right"></i><span>সম্পূর্ণ টেবিল দেখতে পাশে স্ক্রোল করুন</span>';
